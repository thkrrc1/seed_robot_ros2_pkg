...
template<class T>
bool makeBuffCurrentImpl(BuffRaw &buff, int msid) {
    return false;
}

template<>
bool makeBuffCurrentImpl<aero3::SendBuff>(BuffRaw &buff, int msid) {
...
template<>
bool makeBuffCurrentImpl<aero4::SendBuff>(BuffRaw &buff, int msid) {
...
bool makeBuffCurrent(const std::string &protocol, BuffRaw &buff, int msid) {
    if (protocol == "aero3") {
        return makeBuffCurrentImpl<aero3::SendBuff>(buff, msid);
    } else {
        return makeBuffCurrentImpl<aero4::SendBuff>(buff, msid);
    }
}


template<class T>
bool getDataImpl(const BuffRaw &buff, aero_controller_msgs::msg::Current &status) {
    return false;
}
...
template<>
bool getDataImpl<aero3::RecvBuff>(const BuffRaw &buff, aero_controller_msgs::msg::Current &status) {
    auto *respdata = reinterpret_cast<const aero3::RecvBuff*>(buff.data);
    if (respdata->cmd != 0x42) {
        return false;
...
template<>
bool getDataImpl<aero4::RecvBuff>(const BuffRaw &buff, aero_controller_msgs::msg::Current &status) {
