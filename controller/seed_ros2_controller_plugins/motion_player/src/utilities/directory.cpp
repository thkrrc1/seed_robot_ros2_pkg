...
std::string getFileName(const std::string &path, bool stem_only) {
...
//ファイルの絶対パスが返る。
//再帰検索はしない。
std::vector<std::string> getFileNames(std::string folderPath, std::string extension, std::string prefix,bool recursive) {
    ...
    if (std::filesystem::is_directory(path)) {
        ...
        for (auto &file : dir_it) {
            if(std::filesystem::is_directory(file) && recursive){
                auto nexts = getFileNames(file.path().string(),extension,prefix,recursive);
                file_names.insert(file_names.end(), nexts.begin(), nexts.end());
