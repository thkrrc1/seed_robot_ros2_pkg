...
int SerialCommunication::write(uint8_t* data, int len) {
    ...
    if (writed_size > 0) {
        LOG_TRACE_STREAM() << "[" << port << "]" << "[write]" << " size:" << writed_size << rtlogger::hexdump((unsigned char*) data, len) << LOG_END;
    }
...
int SerialCommunication::read_impl(uint8_t* data, int len){
    ...
    if (readed_size > 0) {
        LOG_TRACE_STREAM() << "[" << port << "]" << "[read]" << " size:" << readed_size << rtlogger::hexdump((unsigned char*) data, readed_size) << LOG_END;
    }
