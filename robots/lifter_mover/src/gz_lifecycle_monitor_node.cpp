...
class GazeboLifecycleMonitor : public rclcpp::Node {
public:
  GazeboLifecycleMonitor(const std::vector<std::string> & target_nodes)
  : Node("gazebo_lifecycle_monitor"), launched_(false)
  {
...
int main(int argc, char ** argv) {
  ...
  std::vector<std::string> names = {"lifter_controller", "joint_state_broadcaster"};
  auto node = std::make_shared<GazeboLifecycleMonitor>(names);
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}
